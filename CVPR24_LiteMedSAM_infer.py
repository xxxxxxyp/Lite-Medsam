...
parser.add_argument(
    ...
    type=str,
    default="/data/xyp/workdir_FL_full/medsam_lite_best_dice.pth",
...
lite_medsam_checkpoint = torch.load(lite_medsam_checkpoint_path, map_location='cpu')
# 提取嵌套在"model"键下的权重字典
model_state_dict = lite_medsam_checkpoint['model']
medsam_lite_model.load_state_dict(model_state_dict)
...
def MedSAM_infer_npz_3D(img_npz_file):
    ...
    # -------------------------- 关键修改：自动生成3D全图box --------------------------
    D, H, W = img_3D.shape  # 获取3D图像的Z轴长度（D）、高度（H）、宽度（W）
    # 3D全图box：x_min=0, y_min=0, z_min=0, x_max=W, y_max=H, z_max=D
    boxes_3D = np.array([[0, 0, 0, W, H, D]])  # 适配3D box格式
...
if __name__ == '__main__':
    ...
    for img_npz_file in tqdm(img_npz_files):
        start_time = time()
        if basename(img_npz_file).startswith('PET'):
            print("3D Mode\n")
