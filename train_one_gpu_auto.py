...
if __name__ == "__main__":
    ...
    wandb.init(project="DLBCL2FL", name="FL_auto",config={
        "epochs": num_epochs,
    ...
    class NpyDataset(Dataset): 
        def __init__(self, data_root, image_size=256, bbox_shift=5, data_aug=False):
            ...
            self.bbox_path = join(data_root, "bboxes")  # 新增：bbox保存路径（和预处理程序一致）
        ...
        def __getitem__(self, index):
            ...
            # --- 关键：稳健处理无病灶情况（label_ids 为空） ---
            unique_ids = np.unique(gt)
            label_ids = unique_ids[unique_ids != 0]  # 排除背景 0

            if label_ids.size == 0:
                # 该切片无任何标注
                # 两种常见策略：
                # 1) 如果希望保留空样本作为 negative example：返回全 0 的 gt2D，并确保 get_bbox 能处理全零（get_bbox 中有兜底返回全图的逻辑）
                # 2) 如果不希望保留：在 __init__ 里过滤掉这类样本（推荐）
                # 这里采用第1种：显式返回全 0 mask（不会错误地将整图当作正样本）
                gt2D = np.zeros_like(gt, dtype=np.uint8)
            else:
                # 原逻辑：随机选一个非 0 标签作为训练目标
                chosen = random.choice(label_ids.tolist())
                gt2D = np.uint8(gt == chosen)
            ...
            bboxes = get_bbox(
                ...
                gt2D=gt2D,
                img_name=img_name,  # 新增：传递图像名
                bbox_path=self.bbox_path,  # 新增：传递bbox保存路径
