...
parser.add_argument(
    '-data_root',
    type=str,
    default="/data/xyp/下载/data/npz/FL/val_0",
    help='root directory of the data',
)
parser.add_argument(
    '-pred_save_dir',
    type=str,
    default="/data/xyp/下载/data/npz/FL/val_infer",
    help='directory to save the prediction',
)
parser.add_argument(
    '-medsam_lite_checkpoint_path',
    type=str,
    default="/data/xyp/workdir_FL_0/medsam_lite_best.pth",
...
if medsam_lite_checkpoint_path.find("best")==-1:
        ...
        for i in range(img_3D.shape[0]):
            ...
            for label_id in label_ids:
                ...
                if np.sum(gt2D_padded) > 0:
                    ...
                    # x1, y1, x2, y2 = resize_box(box, (newh, neww), (H, W))
                    # full_mask[y1:y2, x1:x2] = sam_mask  # 贴回原图
                    # seg_3D[i] = full_mask
                    box_list[i][label_id] = box

        label_ids = np.unique(gt_3D)[1:]
        nii_name=npz_name[:-4]+".nii.gz"
        nii_image=sitk.GetImageFromArray(seg_3D)
        nii=sitk.GetImageFromArray(img_3D)
        sitk.WriteImage(nii_image,join(pred_save_dir,task_folder,"nii",nii_name))
        sitk.WriteImage(nii,join(pred_save_dir,task_folder,"image",nii_name))
...
if __name__ == '__main__':
    ...
    directory = Path(join(pred_save_dir,gt_path_files[0].split('/')[-2],"nii"))
    directory.mkdir(parents=True, exist_ok=True)
    directory = Path(join(pred_save_dir,gt_path_files[0].split('/')[-2],"image"))
    directory.mkdir(parents=True, exist_ok=True)
