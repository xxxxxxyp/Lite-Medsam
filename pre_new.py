...
parser.add_argument("-modality", type=str, default="PET", help="CT or MR, [default: CT]")
parser.add_argument("-anatomy", type=str, default="",
...
parser.add_argument("-img_path", type=str, default="/data/xyp/下载/data/Task501_LymphomaPET/images",
                    help="Path to the nii images, [default: /data/xyp/下载/data/fl/image]")
parser.add_argument("-gt_path", type=str, default="/data/xyp/下载/data/Task501_LymphomaPET/labels",
                    help="Path to the ground truth, [default: /data/xyp/下载/data/dlbcl/label]")
parser.add_argument("-output_path", type=str, default="/data/xyp/下载/data/new",
...
# 新增选项：保留整体体积或保留上下文切片
parser.add_argument("--keep_full_volume", default=True, action="store_true",
                    help="Do not crop by gt, keep full volume (default: False)")
parser.add_argument("--context_slices", type=int, default=0,
                    help="When cropping by gt, expand the z-range by this many slices on each side (default: 0)")
...
keep_full_volume = args.keep_full_volume
context_slices = args.context_slices

"""3D 高斯去噪（直接处理原始 PET 像素值）"""
def pet_denoise_3d(raw_data, sigma=1.0):
    denoised = np.zeros_like(raw_data)
    for z in range(raw_data.shape[0]):
        denoised[z] = cv2.GaussianBlur(raw_data[z], (0, 0), sigmaX=sigma)
    return denoised
...
def preprocess(name, npz_path):
    ...
    if tumor_id is not None:
        ...
        gt_data_ori[tum_inst > 0] = (
            tumor_inst[tum_inst > 0] + np.max(gt_data_ori) + 1
        )

    # find non-zero slices
    coords = np.where(gt_data_ori > 0)
    if len(coords) > 0 and coords[0].size > 0:
        z_index = np.unique(coords[0])
    else:
        z_index = np.array([], dtype=int)

    # 如果选择保留整张体积，则覆盖 z_index
    if keep_full_volume:
        z_index = np.arange(gt_data_ori.shape[0], dtype=int)
    # 否则如果需要保留上下文切片，扩展范围
    elif context_slices > 0 and z_index.size > 0:
        zmin = max(0, int(z_index.min()) - context_slices)
        zmax = min(int(gt_data_ori.shape[0]) - 1, int(z_index.max()) + context_slices)
        z_index = np.arange(zmin, zmax + 1, dtype=int)

    if len(z_index) > 0:
        # crop the ground truth with selected slices
        ...
        if modality == "CT":
            ...
            #image_data_pre = pet_denoise_3d(image_data_pre, sigma=1.0)
            # only consider positive voxels for percentile
            if np.any(image_data > 0):
                lower_bound, upper_bound = np.percentile(
                    image_data[image_data > 0], 0.5
                ), np.percentile(image_data[image_data > 0], 99.5)
            else:
                lower_bound, upper_bound = np.min(image_data), np.max(image_data)
            image_data_pre = np.clip(image_data, lower_bound, upper_bound)
            image_data_pre = (
                (image_data_pre - np.min(image_data_pre))
                / (np.max(image_data_pre) - np.min(image_data_pre))
