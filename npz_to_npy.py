...
parser.add_argument("-npz_dir", type=str, default="/data/xyp/下载/data/npz/FL/val_0",
                    help="Path to the directory containing preprocessed .npz data, [default: data/npz/MedSAM_train/CT_Abd]")
parser.add_argument("-npy_dir", type=str, default="/data/xyp/下载/data/npy/FL/val_0",
    ...
    if len(gts.shape) > 2: ## 3D image
        for i in range(imgs.shape[0]):
            ...
            if img_01.shape[:2] == gt_i.shape:
                ...
                print(f"{npz_name} img shape{img_01.shape} not match gt shape{gt_i.shape}")
    else: ## 2D image
        if len(imgs.shape) < 3:
